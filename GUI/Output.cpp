...
{
	pWind->DrawImage("images\\MenuItems\\colors.jpg", 0 , UI.ToolBarHeight *(ITM_SAVE_TYPE +1), UI.MenuActionWidth -3, 7* UI.ToolBarHeight);
}
...
void Output::selectFillColor(Point p , color & fillColor , bool & isFilled) const {
	...
	if (p.x > 0 && p.x < UI.MenuActionWidth) {
		int selectedColor = ( (p.y - ITM_SAVE_TYPE *UI.MenuActionWidth ) / UI.MenuActionWidth );
